...
export const Calculator = ({
	...
	const [selectedCup, setSelectedCup] = useState<Cup>(cupData.sort((a, b) => a.color.localeCompare(b.color))[0]);
	...
	useEffect(() => {
		...
		const imprintSelect = document.getElementById("imprintType") as HTMLSelectElement | undefined;
		if (imprintSelect) {
		...
		if (cupConfig.imprintType === "direct_print") {
			for (const amount of [amounts.amount1, amounts.amount2, amounts.amount3]) {
				if (amount && amount < 72) {
					...
					if (Math.max(amounts.amount1 || 0, amounts.amount2 || 0, amounts.amount3 || 0) < 216) {
	...
	const amountAlerts = (amount: number | null, inputId: number) => {
		...
		if (amount >= 50 && amount < 72) {
			toast.info(lang === "1" ? "Lepsza cena przy 72 sztukach" : "You get better price for 72 pieces");
			return;
		}
		if (amount >= 100 && amount < 108) {
			toast.info(lang === "1" ? "Lepsza cena przy 108 sztukach" : "You get better price for 108 pieces");
			return;
		}
		if (amount >= 200 && amount < 216) {
			toast.info(lang === "1" ? "Lepsza cena przy 216 sztukach" : "You get better price for 216 pieces");
			return;
		}
		if (amount >= 500 && amount < 504) {
			toast.info(lang === "1" ? "Lepsza cena przy 504 sztukach" : "You get better price for 504 pieces");
			return;
		}
		if (amount >= 1000 && amount < 1008) {
			toast.info(lang === "1" ? "Lepsza cena przy 1008 sztukach" : "You get better price for 1008 pieces");
			return;
		}
		if (amount >= 2500 && amount < 2520) {
			toast.info(lang === "1" ? "Lepsza cena przy 2520 sztukach" : "You get better price for 2520 pieces");
			return;
		}
		if (amount >= 5040) {
			toast.info(
		...
		<div className="flex flex-col items-center group-data-[embed=true]:pt-24 pt-12 pb-16">
			<div className="flex flex-row w-full items-center justify-center gap-8">
				...
				<div className="flex flex-col">
					...
					<br />
					{!noPrices ? (
						<div className="flex flex-col relative w-80 h-44">
							...
							{clientPriceUnit === "zł" && <p>{lang === "1" ? "Koszt transportu: " : "Transport cost: "}</p>}
						...
						<div className="flex flex-col relative w-80 h-32">
							<p className="text-center text-xl w-96 mt-6 ml-6">
								{lang === "1"
									? "W celu otrzymania kalkulacji skontaktuj się z działem handlowym."
									: "Contact sales department to get a calculation."}
							</p>
						</div>
					)}
				</div>
			...
			<div className="flex flex-row">
				<div className="mr-4 mt-4 flex flex-col gap-2 items-end">
					<div className="flex flex-row gap-2 items-center">
						...
						<select
							onChange={(e) => {
								...
								setSelectedCup(cupData.find((cup) => cup.code === e.target.value) as Cup);
								// reset input/select elements
							}}
							defaultValue={cupData.sort((a, b) => a.color.localeCompare(b.color))[0].code}
					...
					{!noPrices && (
						<>
							<div className="relative pt-5">
								{lang === "1" ? "Ilość: " : "Amount: "}
								<div className="flex flex-row absolute ml-2 top-0 left-[11vw] pt-5">
							...
							<div className="flex flex-col gap-2 w-[386px] bg-slate-100 p-4">
										...
										<select
											...
											onChange={(e) => {
												...
												setCupConfig({
													...
													imprintType: "",
													imprintColors: 1,
													trend_color: e.target.value as CupConfigInterface["trend_color"],
													pro_color: false,
													soft_touch: false,
													nadruk_wewnatrz_na_sciance: 0,
													nadruk_na_uchu: false,
											...
											className="border w-max border-[#bbb] bg-slate-50 text-black px-2 py-[2px] rounded-md"
										>
											<option value="">{lang === "1" ? "Bez Trend Color" : "No Trend Color"}</option>
											<option value="inside">{lang === "1" ? "Wewnątrz" : "Inside"}</option>
											<option value="outside">{lang === "1" ? "Zewnątrz" : "Outside"}</option>
											<option value="both">{lang === "1" ? "Wenątrz i na zewnątrz" : "Inside and outside"}</option>
											{selectedCup.trend_color_lowered_edge && (
												<option value="lowered_edge">
													{lang === "1" ? "Na zewnątrz z obniżonym rantem" : "Outside with lowered edge"}
												</option>
											)}
										</select>
									</div>
								...
								{selectedCup.soft_touch && (
									<div className="flex flex-row justify-between items-center">
										...
										<select
											...
											className="border w-max border-[#bbb] bg-slate-50 text-black px-2 py-[2px] rounded-md"
										>
											<option value="">{lang === "1" ? "Bez Soft Touch" : "No Soft Touch"}</option>
								...
								<div className="flex flex-row justify-between items-center">
									...
									>
										...
										{selectedCup.polylux && !forbidden.polylux && !selectedCup.digital_print && (
											<>
												<option value="polylux_1">Polylux {lang === "1" ? "1 strona" : "1 side"}</option>
												<option value="polylux_2">Polylux {lang === "1" ? "2 strony" : "2 sides"}</option>
												<option value="polylux_round">Polylux {lang === "1" ? "tapeta" : "wallpaper"}</option>
											</>
										)}
										{selectedCup.deep_effect && !forbidden.deep_effect && (
											<>
												<option value="deep_effect_1">Deep effect {lang === "1" ? "1 strona" : "1 side"}</option>
												<option value="deep_effect_2">Deep effect {lang === "1" ? "2 strony" : "2 sides"}</option>
											</>
										)}
										{selectedCup.deep_effect_plus && !forbidden.deep_effect_plus && (
											<>
										...
										{selectedCup.digital_print && !forbidden.digital_print && (
											<option value="digital_print">
												{lang === "1" ? "Nadruk cyfrowy - pełny kolor" : "Digital print - full color"}
											</option>
										)}
									</select>
								</div>
								{((cupConfig.imprintType &&
									...
									<div
										className={`flex flex-row justify-between items-center relative ${
											["digital_print", ""].includes(cupConfig.imprintType) && anyAdditionalPrint(cupConfig) && "mb-3"
										}`}
									>
							...
							<span>{lang === "1" ? "Zdobienia/nadruki dodatkowe:" : "Additional decorations/prints:"}</span>
							<div className="flex flex-col flex-wrap gap-1 accent-[#009E60] bg-slate-100 pt-4 pb-2">
								{selectedCup.nadruk_wewnatrz_na_sciance && !forbidden.nadruk_wewnatrz_na_sciance && (
									<div className="flex flex-row gap-2 items-center mx-4">
										...
										/>
										<p className="py-[2px]">
											{lang === "1" ? "Nadruk wewnątrz na ściance" : "Print on the wall inside"}
										</p>
										{cupConfig.nadruk_wewnatrz_na_sciance ? (
											<select
												...
												onChange={(e) =>
													setCupConfig({
														...cupConfig,
														nadruk_wewnatrz_na_sciance: parseInt(e.target.value) as 1 | 2 | 3 | 4,
													})
												}
								...
								{selectedCup.nadruk_na_uchu && !forbidden.nadruk_na_uchu && (
									<div className="flex flex-row gap-2 items-center mx-4">
										<input
											...
											className="cursor-pointer"
										/>
										<p className="py-[2px]">{lang === "1" ? "Nadruk na uchu" : "Print on the handle"}</p>
									</div>
								)}
								...
								{selectedCup.nadruk_na_dnie && cupConfig.trend_color !== "inside" && !forbidden.nadruk_na_dnie && (
									<div className="flex flex-row gap-2 items-center mx-4">
										<input
											...
											className="cursor-pointer"
										/>
										<p className="py-[2px]">{lang === "1" ? "Nadruk na dnie (wewn.)" : "Print on the bottom inside"}</p>
									</div>
								)}
								{selectedCup.nadruk_przez_rant && !forbidden.nadruk_przez_rant && (
									<div className="flex flex-row gap-2 items-center mx-4">
										<input
											...
											className="cursor-pointer"
										/>
										<p className="py-[2px]">{lang === "1" ? "Nadruk przez rant" : "Over the rim imprint"}</p>
									</div>
								)}
								{selectedCup.nadruk_apla && !forbidden.nadruk_apla && (
									<div className="flex flex-row gap-2 items-center mx-4">
										<input
											...
											className="cursor-pointer"
										/>
										<p className="py-[2px]">{lang === "1" ? "Nadruk apla" : "Apla print"}</p>
									</div>
								)}
								{selectedCup.nadruk_dookola_pod_uchem && !forbidden.nadruk_dookola_pod_uchem && (
									<div className="flex flex-row gap-2 items-center mx-4">
										<input
											type="checkbox"
										...
										/>
										<p className="py-[2px]">
											{lang === "1" ? "Nadruk dookoła (pod uchem)" : "Print around (under the handle)"}
										</p>
									</div>
								)}
								{((selectedCup.nadruk_zlotem_do_25cm2 && !forbidden.nadruk_zlotem_25) ||
									...
									<div className="flex flex-row gap-2 items-center mx-4">
										<input
											...
											className="cursor-pointer"
										/>
										<p className="py-[2px]">{lang === "1" ? "Nadruk złotem" : "Gold print"}</p>
								...
								{selectedCup.personalizacja && !forbidden.personalizacja && (
									<div className="flex flex-row gap-2 items-center mx-4">
										<input
											...
											className="cursor-pointer"
										/>
										<p className="py-[2px]">{lang === "1" ? "Personalizacja" : "Personalization"}</p>
									</div>
								)}
								{((selectedCup.zdobienie_paskiem_bez_laczenia && !forbidden.zdobienie_paskiem_bez_laczenia) ||
									(selectedCup.zdobienie_paskiem_z_laczeniem && !forbidden.zdobienie_paskiem_z_laczeniem)) && (
									<div className="flex flex-row gap-2 items-center mx-4">
										<input
											type="checkbox"
											...
											className="cursor-pointer"
										/>
										<p className="py-[2px]">{lang === "1" ? "Zdobienie paskiem" : "Decoration with stripe"}</p>
										{cupConfig.zdobienie_paskiem ? (
											<select
												...
												onChange={(e) => {
													setCupConfig({
														...cupConfig,
														zdobienie_paskiem: e.target.value as "bez_laczenia" | "z_laczeniem",
													});
												}}
											...
											>
												...
												{selectedCup.zdobienie_paskiem_z_laczeniem && !forbidden.zdobienie_paskiem_z_laczeniem && (
													<option value="z_laczeniem">{lang === "1" ? "Z łączeniem" : "With connection"}</option>
												)}
											</select>
										) : (
											<div className="w-48" />
								...
								{selectedCup.nadruk_na_powloce_magicznej_1_kolor && !forbidden.nadruk_na_powloce_magicznej_1_kolor && (
									<div className="flex flex-row gap-2 items-center mx-4">
										<input
											type="checkbox"
								...
								{((selectedCup.zdobienie_tapeta_na_barylce_I_stopien_trudnosci &&
									...
									<div className="flex flex-row gap-2 items-center mx-4">
										...
										/>
										<p className="py-[2px]">
											{lang === "1" ? "Zdobienie tapetą na baryłce" : "Decoration with tape on the barrel"}
										</p>
										{cupConfig.zdobienie_tapeta_na_barylce ? (
											<select
												...
												onChange={(e) => {
													setCupConfig({
														...cupConfig,
														zdobienie_tapeta_na_barylce: e.target.value as "I_stopien" | "II_stopien",
													});
												}}
												...
												{selectedCup.zdobienie_tapeta_na_barylce_I_stopien_trudnosci &&
													!forbidden.zdobienie_tapeta_na_barylce_I_stopien && (
														<option value="I_stopien">
															{lang === "1" ? "I stopień trudności" : "I degree of difficulty"}
														</option>
													)}
												{selectedCup.zdobienie_tapeta_na_barylce_II_stopien_trudnosci &&
													!forbidden.zdobienie_tapeta_na_barylce_II_stopien && (
														<option value="II_stopien">
															{lang === "1" ? "II stopień trudności" : "II degree of difficulty"}
														</option>
													)}
											</select>
										) : (
								...
								{selectedCup.naklejka_papierowa_z_nadrukiem && !forbidden.naklejka_papierowa_z_nadrukiem && (
									<div className="flex flex-row gap-2 items-center mx-4">
										<input
											type="checkbox"
								...
								{selectedCup.wkladanie_ulotek_do_kubka && !forbidden.wkladanie_ulotek_do_kubka && (
									<div className="flex flex-row gap-2 items-center mx-4">
										<input
											type="checkbox"
					...
					<div className="mt-5">
						<div className="flex flex-row ml-64 gap-[28px] items-center relative">
							<div className="inline-flex flex-col absolute -left-64 bg-slate-100 px-4 pb-4 pt-2">
								...
								>
									<option value="">{lang === "1" ? "Opakowanie zbiorcze" : "Bulk packaging"}</option>
									<option value="singular">{lang === "1" ? "Kartoniki jednostkowe" : "Single boxes"}</option>
									{selectedCup.category !== "filiżanka" && (
										<>
											<option value="6pack_wykrojnik">
												{lang === "1" ? "6-pak z wykrojnika" : "6-pack from a die"}
											</option>
											<option value="6pack_klapowy">{lang === "1" ? "6-pak klapowy" : "6-pack flap"}</option>
										</>
									)}
								</select>
							</div>
					...
					<div className="mt-5 flex justify-center gap-8">
						...
						<button
							...
							onClick={async () => {
								...
								if (res) {
									toast.success(lang === "1" ? "Skopiowano do schowka" : "Copied to clipboard");
									return;
								}
